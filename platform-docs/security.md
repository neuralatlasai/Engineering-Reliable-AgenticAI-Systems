...
| Asset changed after discovery | Discovery retains and hashes the verified bytes; publication writes those retained bytes rather than reopening the source path | Build from an immutable commit so the published corpus and reviewed revision remain operationally identical. |
